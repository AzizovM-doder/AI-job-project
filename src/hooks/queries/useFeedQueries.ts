...
export const useFeedQueries = () => {
  ...
  const enrichWithProfiles = async <T extends { userId: number; repostSourceUserId?: number | null; authorName?: string; authorAvatar?: string | null; authorHeadline?: string | null; user?: any }>(
    items: T[]
  ): Promise<T[]> => {
    if (!items.length) return items;

    const userIdsInItems = items.map((i) => i.userId);
    const repostUserIds = items.map((i) => i.repostSourceUserId).filter(Boolean) as number[];
    const uniqueUserIds = Array.from(new Set([...userIdsInItems, ...repostUserIds])).filter(Boolean);
